...
========
    ...
    where
        ...
        - Ifc is a :class:`~interface.Interface` instance
    ...
    The :class:`~interface.Interface` API supports implementation of an optical action, such as refraction, reflection, scatter, diffraction, etc. The :class:`~interface.Interface` may be realized as a physical profile separating the adjacent gaps or an idealized object, such as a thin lens or 2 point HOE.
    ...
    The combination of Interfaces and Gaps completely define the path of a ray through the optical model. For convenience and efficiency when doing optical calculations, it is useful to extract a subset of data that is required for ray tracing. The :meth:`~sequential.SequentialModel.path` in :class:`~sequential.SequentialModel` returns a Python generator that is used to drive the ray tracing process.
