...
====================
   ...
   Real ray tracing is a fundamental operation in optical design and analysis. RayOptics has a layered architecture for the ray trace engine. The low level interface can be used directly or through higher level interfaces. 
   ...
   Sequential ray tracing in RayOptics by default is configured to ray trace the :attr:`~opticalmodel.OpticalModel.seq_model` of the |opm|. The :func:`~raytr.raytrace.trace` function is the low level interface to the ray trace system. It's arguments include a |sm| and the ray starting point, direction and wavelength.
